...
class Match(models.Model):
    ...
    home_team_tries     = models.IntegerField( null=True, blank=True, default=0)
    ...
    away_team_tries     = models.IntegerField( null=True, blank=True, default=0 )
    ...
    jokerbet_text       = models.CharField(max_length=50, default='')
    jokerbet_response   = models.NullBooleanField()
    jokerbet_win_pts    = models.IntegerField(default=0)
    jokerbet_lose_pts   = models.IntegerField(default=0)
...
def update_team_points(sender, instance, **kwargs):
    ...
    """
    # Do not update team's point if the match wasn't already played
    utc = pytz.UTC
    date_now = utc.localize(datetime.datetime.now())
    if date_now > instance.date:
...
def update_team_stats(sender, instance, **kwargs):
    ...
    # Do not update team's stats if the match wasn't already played
    utc = pytz.UTC
    date_now = utc.localize(datetime.datetime.now())
    if date_now > match.date:
